export const navbar = () => {
  return `
    <div id="nav_container">
    <div id="brand">
      <a href="./index.html"><img  src="./img/website logo.png" alt="" /></a>
    </div>
    <div id="menu_main">
      <div id="workouts">
        <a href="">WORKOUTS <i class="fa-solid fa-caret-down"></i></a>

        <div id="workout_inner_pages">
          <div class="inner_page">
            <div id="unique-1" class="logo_name_container">
              <div class="workout_info">
                <img
                  src="https://www.nicepng.com/png/full/47-478784_gymnastics-svg-dancer-silhouette-yoga-poses-silhouette-png.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">WORKOUTS VIDEOS</p>
            </div>
            
            
            <div id="unique-2" class="logo_name_container">
              
              <div class="workout_info">
                <img
                  src="https://cdn-icons-png.flaticon.com/512/2313/2313181.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">CUSTOM WORKOUTS</p>
            
            </div>
          
          
          </div>
        </div>

      </div>
      <div id="programs" id="workout2">
        <a href="">PROGRAMS <i class="fa-solid fa-caret-down"></i></a>

        <div id="program_inner_pages">
          <div class="inner_page">
           


            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                  src="http://cdn.onlinewebfonts.com/svg/img_464400.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">workout  programs</p>
            </div>

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                  src="https://static.thenounproject.com/png/668842-200.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">meal plans</p>
            </div>

            
            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                  
                  src="https://icon-library.com/images/tasks-icon/tasks-icon-24.jpg"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">pilot programs</p>
            </div>
            
            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://www.veryicon.com/download/png/education-technology/passenger-flow-analysis-icon-library/edit-48?s=256"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">routines</p>
            </div>
          </div>
        </div>
      </div>
      <div id="healthy_living">
        <a href="">
          HEALTHY LIVING <i class="fa-solid fa-caret-down"></i
        ></a>

        <div id="healthy_living_inner_pages">
          <div class="inner_page">

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://www.freeiconspng.com/uploads/brain-gears-icon-7.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">Expert Articles</p>
            </div>

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://www.pngall.com/wp-content/uploads/8/Recipe-PNG-Free-Image.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">healthy receipes</p>
            </div>


            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://www.policeoneacademy.com/wp-content/uploads/2019/05/Health-and-Wellness.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">routines</p>
            </div>


            
          </div>
        </div>

        
      </div>
      <div id="community">
        <a href="">COMMUNITY <i class="fa-solid fa-caret-down"></i></a>

        <div id="community_inner_pages">
          <div class="inner_page">
            

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://www.freeiconspng.com/thumbs/live-chat-icon/live-chat-icon-18.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">community</p>
            </div>


            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://cdn-icons-png.flaticon.com/512/2125/2125457.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">blog</p>
            </div>

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://cdn-icons-png.flaticon.com/512/40/40058.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">whats new</p>
            </div>

            
            
          </div>
        </div>



      </div>
      <div id="about">
        <a href="">ABOUT <i class="fa-solid fa-caret-down"></i></a>


        <div id="about_inner_pages">
          <div class="inner_page">


            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://excelautomationinc.com/wp-content/uploads/2019/05/about.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">about</p>
            </div>

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://cdn-icons-png.flaticon.com/512/54/54560.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">careers</p>
            </div>

            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://cdn-icons-png.flaticon.com/512/1987/1987955.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">tutorials</p>
            </div>
            <div  class="logo_name_container">
              <div class="workout_info">
                <img
                src="https://icons.veryicon.com/png/o/miscellaneous/site-icon-library/team-28.png"
                  alt=""
                  class="logo"
                />
              </div>
              <p class="logoName">our team</p>
            </div>

           
          </div>
        </div>


      </div>
      <div id="store"><a href="">STORE</a></div>
      <div id="membership"><a href="">MEMBERSHIP</a></div>
    </div>
    <div id="menu_controls">
          <div id="login">
            <div>
              <span class="bl"> Hi! Sign In </span>
            </div>
            <div>
              <span> MY FITNESS <i class="fa-solid fa-caret-down"></i></span>
            </div>
          </div>
          <div id="search_mid">
            <input  type="text" placeholder="Search here">
            <div id="search"><i class="fa-solid fa-magnifying-glass"></i></div>
          </div>
          
          </div>
      <div id="cart">
        <i class="fa-solid fa-bag-shopping"></i>
      </div>
    </div>
  </div>


 
    
    
    `;
};
